...
FOR %%a IN (*.mp4) DO "C:\Program Files\ffmpeg\bin\ffmpeg.exe" -i "%%~na.mp4" -i "%%~na.ass" -c:v copy -c:a copy -c:s srt -metadata:s:s:0 language=rus -metadata:s:s:0 handler="Russian" "new_%%~na.mkv"
