FOR %%a IN (*.mkv) DO "C:\Program Files\ffmpeg\bin\ffmpeg.exe" -i "%%~na.mkv" -i "%%~na.ass" -c:s copy -codec:v copy -codec:a libmp3lame -b:a 320k -map 0:v:0 -map 0:a:0 -map 1:0 "new_%%~na.mkv"
