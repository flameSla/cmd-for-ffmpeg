...
echo 2 - hevc_qsv
echo 3 - av-1
echo 4 - hevc_nvenc -b:v 1500k
echo.
choice /c 1234 /m "default - 1" /t 10 /d 1
if "%ERRORLEVEL%"=="1" ( FOR /F "usebackq delims==" %%a IN (files.tmp) DO "C:\Program Files\ffmpeg\bin\ffmpeg.exe" -i %%a -map 0:v -map 0:a -map 0:s %resize%-c:v hevc_nvenc -preset slow -qp 26 -c:a copy -c:s copy "%%~dpahevc_nvenc_%%~na.mkv" )
...
if "%ERRORLEVEL%"=="4" ( FOR /F "usebackq delims==" %%a IN (files.tmp) DO "C:\Program Files\ffmpeg\bin\ffmpeg.exe" -i %%a %resize%-c:v hevc_nvenc -preset slow -b:v 1500k -c:a copy -c:s copy "%%~dpahevc_nvenc_%%~na.mkv" )
