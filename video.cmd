...
:answer1
FOR /F "usebackq delims==" %%a IN (files.tmp) DO "C:\Program Files\ffmpeg\bin\ffmpeg.exe" -i %%a %resize%-c:v hevc_nvenc -preset slow -qp 26 -c:a copy -c:s copy "%%~dpahevc_nvenc_%%~na.mkv"
goto exit

:answer2
set /p vbitrate=Enter the video bitrate (1200k):
FOR /F "usebackq delims==" %%a IN (files.tmp) DO "C:\Program Files\ffmpeg\bin\ffmpeg.exe" -i %%a %resize%-c:v hevc_nvenc -preset slow -b:v %vbitrate% -c:a copy -c:s copy "%%~dpahevc_nvenc_%%~na.mkv"
goto exit

:answer3
set /p vbitrate=Enter the video bitrate (1200k):
set /p abitrate=Enter the audio bitrate (192k):
FOR /F "usebackq delims==" %%a IN (files.tmp) DO "C:\Program Files\ffmpeg\bin\ffmpeg.exe" -i %%a %resize%-c:v hevc_nvenc -preset slow -b:v %vbitrate% -c:a ac3 -ac 2 -b:a %abitrate% -c:s copy "%%~dpahevc_nvenc_%%~na.mkv"
goto exit
